    ...
    $.getJSON(check_in, function (json) {
        ...
        //} else if (m > 0) 
        {
            ...
            var opt = {
                type: "basic",
                title: "❤️",
                message: "Bruce Loves You❤️",
                iconUrl: "icon_48.png"
            }
            var notification = chrome.notifications.create(url, opt, function (notifyId) {
                return notifyId
            });
...
chrome.extension.onRequest.addListener(function (request, sender, sendResponse) {
    ...
    switch (request.method) {
        ...
        case 'openSettings':
            chrome.tabs.create({
                url: chrome.runtime.getURL("options.html") + '#' + request.anchor
            });
            sendResponse({
                data: {
                    tabid: sender.tab.id
                ...
                getOnlineWebsterCollegiate(term, function (word, json) {
                    ...
                    chrome.tabs.sendMessage(tab.id, {
                        ...
                        data: {
                            shanbay: data,
                            webster: {
                                term: json.hw[0].textContent.replace(/\*/g, '·'),
                                defs: defs
                            }
                        }
                    });
