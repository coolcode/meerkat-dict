...
function getWordContent(data, defs) {
    ...
    if (data.learning_id && data.learning_id != 0) {
        html += '<div class="add-btn">' +
            '<a href="#" class="btn" id="shanbay-relearn-btn">Forgot</a>' +
            '<a href="http://www.shanbay.com/review/learning/' + data.learning_id + '" target="_blank" class="btn" id="shanbay-check-btn">View</a>' +
            '<p class="success hide">Success！</p>' +
            '</div>';
    } else {
        html += '<p>' + defs + '</p>' +
            '<div class="add-btn">' +
            '<a href="#" class="btn" id="shanbay-add-btn">Add Note</a>' +
            '<a href="#" target="_blank" class="btn hide" id="shanbay-check-btn">View</a>' +
            '<p class="success hide">Success!</p>' +
            '</div>';
    }
    ...
    chrome.extension.sendRequest({
        ...
        switch (rsp.data.msg) {
            case "success":
                ...
                break;
            case "error":
                $('#shanbay_popover .success').text('Failed, try again later.').removeClass().addClass('failed');
                break;
            default:
        }
    });
